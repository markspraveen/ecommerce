...
const Product = (props) => {
  ...
  const { showCart, addToCart } = props;
  const router = useRouter();
  const { showCart: cartVisible } = useSelector((state) => state.products);
  ...
  return (
    <div className="group group-hover:bg-opacity-60 transition duration-500 relative bg-gray-50 sm:p-28 py-36 px-10 flex justify-center items-center max-h-[500px]">
      <img
...
const mapDispatchToProps = (dispatch) => {
  return {
    setProducts: (products) => dispatch(userActions.setProducts(products)),
    showCart: (show) => dispatch(userActions.showCart(show)),
